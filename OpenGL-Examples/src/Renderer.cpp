...
void Renderer::DrawQuad(const glm::vec2& position, const glm::vec2& size, const glm::vec4& color)
{
	...
	s_Data.QuadBufferPtr->Positions = { position.x, position.y, 0.0f };
	s_Data.QuadBufferPtr->Color = color;
	s_Data.QuadBufferPtr->TexCoords = { 0.0f, 0.0f };
	s_Data.QuadBufferPtr->TexID = textureIndex;
	s_Data.QuadBufferPtr++; // increment the buffer pointer

	s_Data.QuadBufferPtr->Positions = { position.x + size.x, position.y, 0.0f };
	s_Data.QuadBufferPtr->Color = color;
	s_Data.QuadBufferPtr->TexCoords = { 1.0f, 0.0f };
	s_Data.QuadBufferPtr->TexID = textureIndex;
	s_Data.QuadBufferPtr++; // increment the buffer pointer

	s_Data.QuadBufferPtr->Positions = { position.x + size.x, position.y + size.y, 0.0f };
	s_Data.QuadBufferPtr->Color = color;
	s_Data.QuadBufferPtr->TexCoords = { 1.0f, 1.0f };
	s_Data.QuadBufferPtr->TexID = textureIndex;
	s_Data.QuadBufferPtr++; // increment the buffer pointer

	s_Data.QuadBufferPtr->Positions = { position.x, position.y + size.y, 0.0f };
	s_Data.QuadBufferPtr->Color = color;
